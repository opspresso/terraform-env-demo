...
resource "aws_lb_listener_rule" "internal_http--loki" {
  ...
  condition {
    host_header {
      values = [format("loki.demo-in.%s", var.root_domain)]
    }
  }
...
resource "aws_lb_listener_rule" "internal_http--prom" {
  ...
  condition {
    host_header {
      values = [format("prom.demo-in.%s", var.root_domain)]
    }
  }
...
resource "aws_lb_listener_rule" "internal_http--a" {
  ...
  condition {
    host_header {
      values = [format("*.demo-in-a.%s", var.root_domain)]
    }
  }
...
resource "aws_lb_listener_rule" "internal_http--b" {
  ...
  condition {
    host_header {
      values = [format("*.demo-in-b.%s", var.root_domain)]
    }
  }
